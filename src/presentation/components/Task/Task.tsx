...
import { remove } from '../../../redux/features/tasks/tasksSlice';
import { push } from '../../../redux/features/task/taskSlice';
...
export const Task = ({task}: Props) => {
  ...
  return (
    <div className={style.taskWrapper} onClick={chandleClick}>
        <div className='flex-1 overflow-auto'>
            <p className={`font-bold text-lg truncate ${completed?'line-through':''}`}>{title}</p>
            <p className={`truncate ${completed?'line-through':''}`}>{description}</p>
        </div>
        <div>
            <button className='icon' onClick={handleDelete}>
                <svg xmlns="http://www.w3.org/2000/svg"fill="currentColor" version="1.1" id="Capa_1" width="800px" height="800px" viewBox="0 0 482.428 482.429" style={{height: '100%', width: '100%'}}>
                    <g>
                        <g>
